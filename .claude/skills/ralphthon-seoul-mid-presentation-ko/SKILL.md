...
2. 설치된 `slides-grab` 스킬 호출 (Skill 도구 사용)
3. 필수 인터뷰 진행
...
1. `slides-grab` 사용 가능 여부 확인:
   - `slides-grab --help`
   - 또는 `npx slides-grab --help`
2. 없으면 `references/slides-grab-setup.md` 기준으로 설치를 안내합니다.
3. Claude Code용 `slides-grab` 스킬이 없으면 아래 설치를 안내합니다:
   - `npm install slides-grab`
   - `npx playwright install chromium`
4. CLI 사용 가능 상태가 된 뒤 인터뷰를 이어갑니다.
5. 설치가 끝나면 slides-grab CLI를 직접 사용하여 계획/HTML 생성/검증/viewer/editor 흐름을 진행합니다.
...
4. slides-grab CLI를 사용하여 슬라이드 제작을 진행합니다.
5. 슬라이드 작업 경로는 `decks/<deck-name>/`를 사용합니다.
6. `slide-XX.html` 파일을 생성합니다.
7. `npx slides-grab validate --slides-dir <path>`를 실행합니다.
8. 검증이 통과할 때까지 HTML/CSS를 수정합니다.
9. `npx slides-grab build-viewer --slides-dir <path>`를 실행합니다.
10. viewer 위치를 사용자에게 알려줍니다.
11. **HTML 슬라이드가 준비되면 반드시 `npx slides-grab edit --slides-dir <path>`를 실행합니다.** 이 워크플로우에서는 editor 실행이 필수입니다.
...
- 이후 export가 필요하면 `slides-grab`의 export 기능을 별도로 사용합니다.

...
export async function POST(request: Request) {
  ...
  // Authenticated client — RLS ensures user can only see their workspace's messages
  ...
  // Service role needed — RLS doesn't allow authenticated UPDATE
  ...
  const shareUrl = buildShareUrl(request, newToken);
  return NextResponse.json({
    shareToken: newToken,
    shareUrl,
  });
